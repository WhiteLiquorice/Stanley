...
async function run() {
    ...
    await (0, overlay_1.overlayMinimize)(page, 'Navigating to monday.com...');
    await page.waitForTimeout(500);
    await page.goto((0, utils_1.localPage)('saas-pricing.html'));
    await page.waitForTimeout(700);
    await (0, overlay_1.injectOverlayRunning)(page, 'Checking monday.com...');
    // monday.com
    await page.evaluate(() => {
        const card = document.getElementById('card-monday');
        if (card) {
            card.style.transition = 'all 0.4s';
            card.style.borderColor = 'rgba(34,197,94,0.7)';
            card.style.boxShadow = '0 0 40px rgba(34,197,94,0.12)';
        }
    });
    await page.waitForTimeout(900);
    await page.evaluate(() => {
        const price = document.getElementById('monday-price');
        if (price) {
            price.style.transition = 'color 0.4s';
            price.style.color = '#22c55e';
        }
    });
    await (0, overlay_1.overlaySetStatus)(page, 'monday.com: $9/seat ✓');
    await page.waitForTimeout(1500);
    // Asana
    await (0, overlay_1.overlaySetStatus)(page, 'Checking Asana...');
    await page.evaluate(() => {
        const card = document.getElementById('card-asana');
        if (card) {
            card.style.transition = 'all 0.4s';
            card.style.borderColor = 'rgba(34,197,94,0.7)';
            card.style.boxShadow = '0 0 40px rgba(34,197,94,0.12)';
    ...
    await page.waitForTimeout(900);
    await page.evaluate(() => {
        const price = document.getElementById('asana-price');
        if (price) {
            price.style.transition = 'color 0.4s';
            price.style.color = '#22c55e';
        }
    });
    await (0, overlay_1.overlaySetStatus)(page, 'Asana: $10.99/seat ✓');
    await page.waitForTimeout(1500);
    // Notion
    await (0, overlay_1.overlaySetStatus)(page, 'Checking Notion...');
    await page.evaluate(() => {
        const card = document.getElementById('card-notion');
        if (card) {
            card.style.transition = 'all 0.4s';
            card.style.borderColor = 'rgba(34,197,94,0.7)';
            card.style.boxShadow = '0 0 40px rgba(34,197,94,0.12)';
    ...
    await page.waitForTimeout(900);
    await page.evaluate(() => {
        const price = document.getElementById('notion-price');
        if (price) {
            price.style.transition = 'color 0.4s';
            price.style.color = '#22c55e';
        }
    });
    await (0, overlay_1.overlaySetStatus)(page, 'Notion: $15/seat ✓');
    await page.waitForTimeout(1500);
    await (0, overlay_1.overlayDone)(page, 'Comparison ready');
    await page.waitForTimeout(2000);
...
//# sourceMappingURL=04-competitor-pulse.js.map