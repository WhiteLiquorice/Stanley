...
exports.injectOverlayRunning = injectOverlayRunning;
exports.overlayHumanType = overlayHumanType;
exports.overlayType = overlayType;
exports.overlayClickRun = overlayClickRun;
exports.overlayShowPlan = overlayShowPlan;
exports.overlayClickConfirm = overlayClickConfirm;
exports.overlayMinimize = overlayMinimize;
exports.overlaySetStatus = overlaySetStatus;
exports.overlayDone = overlayDone;
exports.overlayShowResult = overlayShowResult;
...
async function injectOverlay(page) {
    await page.evaluate((html) => {
        if (document.getElementById('sly-root'))
            return;
...
async function injectOverlayRunning(page, text = 'Stanley is working...') {
    await page.evaluate(({ html, badgeText }) => {
        if (document.getElementById('sly-root'))
            return;
        const container = document.createElement('div');
        container.innerHTML = html;
...
async function overlayHumanType(page, text) {
    await page.evaluate(() => {
        const ta = document.getElementById('sly-textarea');
        if (ta) {
            ta.focus();
            ta.value = '';
        }
...
async function overlayType(page, text, charDelayMs = 30) {
    await page.evaluate(() => {
        const ta = document.getElementById('sly-textarea');
        if (ta) {
            ta.focus();
...
async function overlayClickRun(page) {
    await page.evaluate(() => {
        const btn = document.getElementById('sly-run-btn');
        if (btn) {
        ...
        if (desc)
            desc.textContent = 'Compiling workflow...';
        if (log)
            log.textContent = 'Asking AI to structure steps';
    });
    await page.waitForTimeout(250);
}
// Show plan steps one by one in the overlay
async function overlayShowPlan(page, steps) {
    await page.evaluate(() => {
        ...
        const log = document.getElementById('sly-status-log');
        if (desc)
            desc.textContent = 'Plan ready — review before running';
        if (log)
            log.textContent = 'Awaiting confirmation';
    });
    ...
    for (let i = 0; i < steps.length; i++) {
        ...
        if (step.action === 'navigate')
            ...
            label = `Click "${step.description || step.url}"`;
        else if (step.action === 'type')
            label = `Type "${step.value}" into ${step.description || 'field'}`;
        else if (step.action === 'scrape')
            label = `Collect ${step.description || 'page content'}`;
        else if (step.action === 'wait')
            label = `Wait for page to settle`;
        else if (step.action === 'open_tab')
            label = `📑 Open new tab${step.url ? ': ' + step.url : ''}`;
        else if (step.action === 'switch_tab')
            label = `📑 Switch to tab ${step.index ?? ''}`;
        else if (step.action === 'scroll')
            label = `Scroll through results`;
        else
            label = step.description || step.action;
        const isTab = TAB_ACTIONS.includes(step.action);
        await page.evaluate(({ idx, text, tab }) => {
            const stepsDiv = document.getElementById('sly-steps');
            if (!stepsDiv)
                return;
...
async function overlayClickConfirm(page) {
    await page.evaluate(() => {
        const btn = document.getElementById('sly-confirm-btn');
        if (btn) {
            btn.style.transform = 'scale(0.96)';
            btn.style.opacity = '0.8';
        }
        setTimeout(() => {
            const b = document.getElementById('sly-confirm-btn');
            if (b) {
                b.style.transform = '';
                b.style.opacity = '';
            }
...
async function overlayMinimize(page, text = 'Stanley is working...') {
...
async function overlaySetStatus(page, text) {
    await page.evaluate((t) => {
...
async function overlayDone(page, text = 'Workflow complete') {
    await page.evaluate((t) => {
...
async function overlayShowResult(page, result) {
    await page.evaluate((text) => {
        const doneBadge = document.getElementById('sly-done-badge');
        const popup = document.getElementById('sly-popup');
        const planPanel = document.getElementById('sly-plan-panel');
        const promptCard = document.getElementById('sly-prompt-card');
        const modeRow = document.querySelector('.sly-mode-row');
        ...
        if (statusLog)
            statusLog.textContent = 'All steps finished successfully';
    }, result);
    await page.waitForTimeout(300);
}
