...
async function slowType(page: any, selector: string, text: string, delay = 52) {
...
async function markFilled(page: any, id: string) {
...
async function run() {
  ...
  await overlayMinimize(page, 'Opening Apex supplier portal...');
  await page.waitForTimeout(500);

  await page.goto(localPage('vendor-form.html'));
  await page.waitForTimeout(700);

  await injectOverlayRunning(page, 'Filling form...');

  await overlaySetStatus(page, 'Entering vendor name...');
  await slowType(page, '#vendor-name', 'Bridgeway Solutions LLC');
  await markFilled(page, 'vendor-name');

  await overlaySetStatus(page, 'Entering vendor ID...');
  await slowType(page, '#vendor-id', 'V-2847');
  await markFilled(page, 'vendor-id');

  await overlaySetStatus(page, 'Entering contact name...');
  await slowType(page, '#contact-name', 'Jordan Davis');
  await markFilled(page, 'contact-name');

  await overlaySetStatus(page, 'Entering contact email...');
  ...
  await overlaySetStatus(page, 'Entering work description...');
  ...
  await overlaySetStatus(page, 'Submitting timesheet...');
  await page.evaluate(() => {
    const btn = document.getElementById('submit-btn') as HTMLElement | null;
    if (!btn) return;
  ...
  await overlayDone(page, 'Timesheet submitted ✓');
  await page.waitForTimeout(2000);
