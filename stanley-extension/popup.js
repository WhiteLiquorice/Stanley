...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const runNativeBtn = document.getElementById('run-native-btn');
  ...
  function renderBuilder() {
    ...
    if (builderSteps.length === 0) {
      ...
      if (runNativeBtn) runNativeBtn.disabled = true;
      return;
    }

    builderEmpty.style.display = 'none';
    // Daemon run needs the native host connected; the browser-native run does not —
    // it only needs a valid license session.
    runBuilderBtn.disabled = !(hasValidSession && isConnected);
    if (runNativeBtn) runNativeBtn.disabled = !hasValidSession;
  ...
  // ── Run in Browser (daemon-free native CDP engine) ───────────────────────────
  // Converts the flat builder step list into the {nodes, edges} graph the native
  // branchingEngine.executeGraph consumes. Vault values stay as `vault:Key`; the
  // engine resolves them from the secrets map we pass alongside.
  function builderStepsToWorkflow(steps, name) {
    const nodes = [];
    const edges = [];
    steps.forEach((step, i) => {
      ...
      const data = {};
      switch (step.action) {
        case 'navigate': data.url = step.url; break;
        case 'open_tab': data.url = step.url; if (step.label) data.label = step.label; break;
        case 'click': if (step.selector) data.selector = step.selector; if (step.description) data.description = step.description; break;
        case 'type':
          if (step.selector) data.selector = step.selector;
          if (step.description) data.description = step.description;
          data.value = step.value || '';
          break;
        case 'wait': data.ms = String(step.ms || 1000); break;
        case 'scrape': if (step.selector) data.selector = step.selector; break;
        case 'switch_tab':
        case 'close_tab': data.tab = step.tab != null ? step.tab : step.index; break;
        default: break;
      }
      nodes.push({ id, type: step.action, label: step.action, data });
      if (i > 0) edges.push({ source: `n${i - 1}`, target: id });
    });
    return { id: 'builder-native', name: name || 'Browser Workflow', nodes, edges };
  }

  runNativeBtn.addEventListener('click', () => {
    if (builderSteps.length === 0 || !hasValidSession) return;

    runNativeBtn.disabled = true;
    resultBlock.classList.add('hidden');
    pausePanel.classList.add('hidden');
    statusDesc.textContent = 'Running in your browser (no daemon)…';
    statusLog.textContent = 'Attaching secure input engine…';
    runningBanner.classList.remove('hidden');
    runningBannerText.textContent = 'Stanley is working in your browser…';

    chrome.storage.local.get(['stanley_vault'], (data) => {
      const vault = data.stanley_vault || {};
      const workflow = builderStepsToWorkflow(
        builderSteps,
        'Browser Workflow (' + builderSteps.length + ' steps)'
      );
      lastRunPrompt = workflow.name;
      chrome.runtime.sendMessage(
        { action: 'run_native_workflow', workflow, secrets: vault },
        (response) => {
          runNativeBtn.disabled = false;
          if (chrome.runtime.lastError) {
            statusDesc.textContent = 'Could not start';
            statusLog.textContent = 'Error: ' + chrome.runtime.lastError.message;
            runningBanner.classList.add('hidden');
            return;
          }
  ...
  chrome.runtime.onMessage.addListener((message) => {
    ...
    // ── Browser-native engine events (daemon-free path) ──────────────────────
    } else if (message.action === "native_log") {
      statusLog.textContent = message.log;
      statusDesc.textContent = "Running in your browser…";
    } else if (message.action === "native_complete") {
      statusDesc.textContent = "Workflow Complete";
      statusLog.textContent = "Completed in your browser.";
      runningBanner.classList.add('hidden');
      if (runNativeBtn) runNativeBtn.disabled = false;
      // Native scrape results are an object keyed by node id — flatten for display.
      const scraped = message.result || {};
      const text = Object.values(scraped).filter(Boolean).join('\n\n');
      if (text) showScrapeResult(text, '');
    } else if (message.action === "native_failed") {
      statusDesc.textContent = "Workflow Failed";
      statusLog.textContent = message.error || "Execution failed.";
      runningBanner.classList.add('hidden');
      if (runNativeBtn) runNativeBtn.disabled = false;
