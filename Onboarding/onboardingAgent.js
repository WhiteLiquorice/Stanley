...
exports.StanleyOnboardingAgent = void 0;
 ...
 */
class StanleyOnboardingAgent extends foundationAgent_1.StanleyFoundation {
    constructor(config = {}) {
        super(config);
    }
    /**
     ...
     */
    async extractLegacyData(competitor, loginUrl, credentials) {
        if (!this.page) {
     ...
     */
    async compressActiveViewportDOM() {
        if (!this.page) {
     ...
     */
    async performLogin(competitor, credentials) {
     ...
     */
    async scrapeLegacyDataTables() {
        if (!this.page)
            return [];
        const rawData = await this.page.evaluate(() => {
            const records = [];
            function cleanHeader(txt) {
     ...
     */
    findNodeByRoleAndName(node, role, nameRegex) {
        if (!node)
            return null;
...
exports.StanleyOnboardingAgent = StanleyOnboardingAgent;
