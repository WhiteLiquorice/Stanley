...
class OrchestrationRuntime {
  ...
  async restoreNode(node) { const run = await this.coordinator.store.get(this.uid, this.runId); const state = run?.nodeState?.[node.id]; return ['completed', 'satisfied'].includes(state?.state) ? { completed: true, output: state.output } : { completed: false }; }
  async claimEffect(node, idempotencyKey) { return this.coordinator.claimEffect(this.uid, this.runId, node.id, idempotencyKey); }
