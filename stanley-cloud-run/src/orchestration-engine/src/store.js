...
class FirestoreOrchestrationStore {
  ...
  async claimEffect(uid, runId, effectKey) { return this.db.runTransaction(async (tx) => { const ref = this.ref(uid, runId).collection('effects').doc(effectKey); const snap = await tx.get(ref); if (snap.exists) return false; tx.create(ref, { claimedAt: new Date().toISOString(), expiresAt: new Date(Date.now() + 90 * 86400000) }); return true; }); }
