...
function installConnectorOverlay({ app, db, authenticateUser, resolveAllSecrets, callModel, logger, onPublished }) {
  ...
  const resolveDeclaredSecrets = async (uid, refs = []) => resolveAllSecrets(db, uid, refs);
  app.use('/v1/connectors', async (req, res, next) => { try { req.uid = await authenticateUser(req); next(); } catch (error) { res.status(error.status || 401).json({ success: false, error: error.message }); } }, createConnectorRouter({ service, resolveSecrets: (uid) => resolveDeclaredSecrets(uid), requireUser: (req) => req.uid, onPublished }));
