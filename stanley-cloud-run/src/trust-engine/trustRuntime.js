...
class TrustRuntime {
  constructor({ store, uid, runId, workflow, overrides = {}, resumeCheckpoint = null }) {
    ...
    if (this.resumePlan) {
      this.workflow = {
        ...this.workflow,
        nodes: (this.workflow.nodes || []).map((node) => this.completedNodeIds.has(node.id)
          ? {
              ...node,
              type: 'wait',
              label: `${node.label || node.type} (restored from checkpoint)`,
              data: { ms: '0', trustOriginalType: node.type, trustCheckpointRestored: true },
            }
          : node),
      };
    }
  ...
  async runFailed(error, context = {}) {
    await this.receipt('run_outcome', null, 'failed', {
      error: { name: error?.name, message: error?.message, code: error?.code },
      context: summarizeContext(context),
    });
    if (this.failureRecorded || !this.policy.openExceptionOnFailure) return null;
    this.failureRecorded = true;
    return this.store.openException(this.uid, {
      runId: this.runId,
      workflowId: this.workflowId,
      ...
      summary: error?.message || 'The run failed without an error message.',
      evidence: { error: { name: error?.name, message: error?.message, code: error?.code } },
    });
  }
