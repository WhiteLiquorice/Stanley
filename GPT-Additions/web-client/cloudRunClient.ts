...
export function isCloudRunnerConfigured(): boolean {
...
export async function waitForRun(runId: string, timeoutMs = 10 * 60 * 1000): Promise<CloudRunResult> {
  ...
  throw new Error('Run is still active after the client wait timeout. It remains available in run history.');
}
...
export async function runWorkflowById(
  ...
  if (terminal.has(payload.run.state)) return toResult(payload.run);
  return waitForRun(payload.run.id);
}
