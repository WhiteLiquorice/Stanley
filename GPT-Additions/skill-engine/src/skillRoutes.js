...
function createSkillRouter({ service, requireUser = (req) => req.uid, loadCompilationSource, resolveSecrets }) {
  ...
  router.post('/:skillId/versions/:version/activate', handle(async (uid, req, res) => res.json({ success: true, skill: await service.activate(uid, req.params.skillId, req.params.version) })));
